...
def generate_html(csv_file='data.csv', items_per_page=100):
    items_by_category = {}
    total_items = 0
    ...
    def process_item_cards(items, cat, bg, border, text):
        sections_html = ""
        for row in items:
...
'''
    ...
    for cat, items in items_by_category.items():
        ...
        for p in range(1, total_pages + 1):
                ...
                <div class="search-container" style="margin-bottom: 30px; margin-top: 10px;">
                    <div style="position: relative; max-width: 600px;">
                        <input type="text" id="loraSearchInput" placeholder="現在のページ内を検索... (例: lora名、サイズ)" 
                               style="width: 100%; padding: 15px 20px 15px 45px; border-radius: 30px; border: 2px solid rgba(56, 189, 248, 0.4); 
                                      background: rgba(255, 255, 255, 0.8); font-size: 1.1rem; outline: none; backdrop-filter: blur(10px);
                                      box-shadow: 0 4px 15px rgba(0,0,0,0.05); transition: all 0.3s ease; box-sizing: border-box;">
                        <i class="fas fa-search" style="position: absolute; left: 18px; top: 50%; transform: translateY(-50%); color: rgba(56, 189, 248, 1); font-size: 1.2rem;"></i>
                    </div>
                </div>
            ...
            sections_html += process_item_cards(page_items, cat, bg, border, text)
...
if __name__ == '__main__':
    generate_html(r'K:\GoogleAI\output.csv', items_per_page=100)
