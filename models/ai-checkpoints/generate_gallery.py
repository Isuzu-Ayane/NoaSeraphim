...
def get_lora_link(cat):
    cat_lower = cat.lower()
    if 'flux' in cat_lower: return '../../models/ai-lora/Flux.html'
    if 'illustrious' in cat_lower or 'noobai' in cat_lower: return '../../models/ai-lora/Illustrious.html'
    if 'pony' in cat_lower: return '../../models/ai-lora/Pony.html'
    if 'sdxl' in cat_lower: return '../../models/ai-lora/SDXL.html'
    if 'sd1.5' in cat_lower or 'sd 1.5' in cat_lower: return '../../models/ai-lora/SD1.5.html'
    if 'wan' in cat_lower: return '../../models/ai-lora/WanVideo.html'
    return '../../models/ai-lora/index.html'
...
def load_seo_descriptions(filepath):
    descriptions = {}
    if not os.path.exists(filepath):
        print(f"Warning: SEO descriptions file not found at {filepath}")
        return descriptions
    
    with open(filepath, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Match by model name (base_name)
            name = row.get('モデル名', '').strip()
            desc = row.get('説明文', '').strip()
            if name and desc:
...
def generate_html():
    ...
    seo_csv_path = r'K:\GoogleAI\checkpoint_seo_descriptions.csv'
    image_dir = r'K:\GoogleAI\NoaSeraphim\models\ai-checkpoints\image_master'
    output_html = r'K:\GoogleAI\NoaSeraphim\models\ai-checkpoints\index.html'

    models_info = parse_model_list(model_list_path)
    seo_descriptions = load_seo_descriptions(seo_csv_path)
...
<head>
    ...
    <title>Stable DiffusionおすすめCheckpointモデル比較・出力サンプルギャラリー</title>
    <meta name="description" content="Stable Diffusionの高品質な画像生成に欠かせない、おすすめCheckpointモデル（ベースモデル）の出力結果を徹底比較。アニメ系から実写・リアル系まで、各モデルの得意なジャンルやファイル容量、ダウンロード先リンクをまとめています。">
    <meta name="keywords" content="Stable Diffusion, Checkpoint, おすすめ, モデル, 比較, AI画像生成, Checkpoints">
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "CollectionPage",
      "name": "Stable DiffusionおすすめCheckpointモデル比較ギャラリー",
      "description": "Stable Diffusionの高品質な画像生成に欠かせない、おすすめCheckpointモデル（ベースモデル）の出力結果を徹底比較。",
      "breadcrumb": {
        "@type": "BreadcrumbList",
        "itemListElement": [{
          "@type": "ListItem",
          "position": 1,
          "name": "ホーム",
          "item": "https://noaailabs.com/"
        },{
          "@type": "ListItem",
          "position": 2,
          "name": "MODELS",
          "item": "https://noaailabs.com/models/ai-checkpoints/index.html"
        }]
      }
    }
    </script>
...
<body>
    <nav class="glass-panel centered-nav" style="position: fixed; top: 0; width: 100%; z-index: 1000; background: rgba(15, 23, 42, 0.8); backdrop-filter: blur(10px); border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: center; align-items: center; padding: 10px 0;">
        <ul style="display: flex; list-style: none; gap: 20px; margin: 0; padding: 0;">
            ...
            <li><a href="../../index.html#about" style="color: white; text-decoration: none; font-weight: bold;">AI-GUIDE</a></li>
            <li><a href="../../models/ai-checkpoints/index.html" style="color: #38bdf8; text-decoration: none; font-weight: bold;">MODELS</a></li>
            <li><a href="../../Tools/ai-tools/index.html" style="color: white; text-decoration: none; font-weight: bold;">TOOLS</a></li>
            <li><a href="../../creative/image-song/index.html" style="color: white; text-decoration: none; font-weight: bold;">GALLERY</a></li>
        </ul>
    </nav>
    ...
    <div class="header-title">
        <nav aria-label="breadcrumb" style="margin-bottom: 20px;">
            <ol style="display:flex; justify-content:center; list-style:none; padding:0; gap:8px; font-size:0.95rem; color:var(--text-muted);">
                <li><a href="../../index.html" style="color:var(--accent); text-decoration:none;">ホーム</a> > </li>
                <li aria-current="page" style="color:var(--text-main);">Checkpoints ギャラリー</li>
            </ol>
        </nav>
        <h1>Stable Diffusion おすすめCheckpointモデル比較一覧</h1>
            ...
            <div class="cp-card">
                ...
                <div class="cp-info">
                    ...
                    <div class="cp-description">
                        {seo_descriptions.get(info['base_name'], seo_descriptions.get(info['base_name'].lower(), "モデルの詳細な説明を準備中です。"))}
                    </div>
